    ...
    try:
        ...
        if not current_step or message.lower() in ["participar", "começar", "assinar"]:
            ...
            message_text = {
                ...
                "to": self.phone,
                "type": "interactive" if next_question["type"] in ["quick_reply", "multiple_choice"] and len(next_question["options"]) >= 3 else "text",
                "text": {"body": next_question["text"]} if next_question["type"] not in ["quick_reply", "multiple_choice"] or len(next_question["options"]) < 3 else None,
                "interactive": {
                    "type": "button",
                ...
                } if next_question["type"] in ["quick_reply", "multiple_choice"] and len(next_question["options"]) >= 3 else None
            }
            if next_question["type"] in ["quick_reply", "multiple_choice"] and len(next_question["options"]) < 3:
                message_text["text"]["body"] += self._format_options(next_question)
        ...
        if next_question:
            ...
            message_text = {
                ...
                "to": self.phone,
                "type": "interactive" if next_question["type"] in ["quick_reply", "multiple_choice"] and len(next_question["options"]) >= 3 else "text",
                "text": {"body": f"{confirmation_text}\n\n{next_question['text']}"} if next_question["type"] not in ["quick_reply", "multiple_choice"] or len(next_question["options"]) < 3 else None,
                "interactive": {
                    "type": "button",
