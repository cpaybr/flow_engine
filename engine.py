...
# --- Função para garantir codificação UTF-8 ---
def ensure_utf8(text: Union[str, bytes]) -> str:
    """Força a decodificação correta de caracteres especiais"""
    if isinstance(text, bytes):
        text = text.decode('utf-8', errors='replace')
    return text.encode('utf-8', errors='replace').decode('utf-8')

# --- Validação de CPF (com tratamento UTF-8) ---
...
# --- Configuração de Logs (UTF-8 habilitado) ---
def setup_logger():
    """Configura loggers com suporte a UTF-8"""
    # Logger principal
    logging.basicConfig(
...
async def process_message(phone: str, campaign_id: str, message: str) -> Dict[str, Any]:
    try:
        ...
        campaign_id = ensure_utf8(campaign_id)
        message = ensure_utf8(message).strip()
        ...
        # Processa JSON com garantia de UTF-8
        def safe_json_load(data):
            if isinstance(data, str):
                try:
                    return json.loads(ensure_utf8(data))
        ...
        # Sanitiza todas as perguntas e opções
        questions = []
        for q in flow.get("questions", []):
            safe_q = {
                **q,
                "id": ensure_utf8(str(q.get("id"))),
                "text": ensure_utf8(q.get("text", "")),
                "type": ensure_utf8(q.get("type", "")),
                "options": [ensure_utf8(opt) for opt in q.get("options", [])],
                "condition": ensure_utf8(q["condition"]) if "condition" in q else None
            }
        ...
        log_event("Estado do usuário carregado", {
            ...
            "answers": {k: ensure_utf8(v) for k, v in answers.items()}
        })
        ...
        if current_question["type"] in ["quick_reply", "multiple_choice"]:
            ...
            if message.startswith("opt_"):
                try:
                    ...
                    if 0 <= idx < len(options):
                        selected = ensure_utf8(options[idx])
                        valid_answer = True
            ...
            if response:
                if "cpf" in question_text:
                    ...
                    answers[str(current_question["id"])] = ensure_utf8(response)
                    valid_answer = True
